...
- __Would the calendar(s) that I added to my Google Calendar would be updated if there are changes to the schedule?__
Yeap, absolutely!
When there are changes to the source `session.json`, I'll get a notification from Github and I'll update the calendars by running the program again.

- __Do I need to re-import / re-add the calendars if there are changes to the schedule?__
Nope, you don't have to. The original GCals will be updated automatically, no need to re-add.

- __How often do the GCals get updated?__
I try to update it as soon as I can, as long as life don't get in my way.

- __Do the GCals get updated automatically?__
Nope, the updates currently have to be triggered manually.

Probably if someone donates a Linux instance fpr a forever-online machine, that can run <-Go 1.5, probably.
